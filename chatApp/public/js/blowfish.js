...
/*
 ...
 http://www.ugpl.de/
  ...
  Example Use (Lingo Syntax) ----------------------
  
  bf = NewBlowFish("some key")
  ciphertext = bf.encrypt("some plaintext")
  plaintext = bf.decrypt("some encrypted text")
 
  Example Use (Javascript Syntax) ----------------------
...
function encrypt (t) {
    ...
    for (var i = 0; i < t.length; i += 16) {
        this.xr_par = wordunescape(t.substr(i, 8));
        this.xl_par = wordunescape(t.substr(i + 8, 8));
        encipher(t);
        r += wordescape(this.xr_par) + wordescape(this.xl_par);
    }
    return r;
};
function decrypt (t) {
...
var bf = new Blowfish("1sadashh23hfgddfg456asdasdasd789");
    
var ciphertext = bf.encrypt('Some pLaintexT that could be of variable length and shit');
var plaintext = bf.decrypt(ciphertext);
               
document.writeln(bf);

document.writeln(ciphertext);
